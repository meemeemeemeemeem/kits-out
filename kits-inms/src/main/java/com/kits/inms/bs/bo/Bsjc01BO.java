...
//import com.kits.inms.itf.controller.CallController;

@Controller
public class Bsjc01BO extends HibernateDao {
	...
	public void getCustNo(Page<Tbcm011> page, Map<String, Object> map)
		...
		if (map != null) {
			...
			if (AppUtils.isNotEmpty(createDate)) {
		...
		hql += " and defs02 <> 'Y' order by createDate desc ";
	...
	public String saveTbbs016(Tbbs016 tbcm011) {
		try {
			...
			if (entityState.equals(EntityState.NEW)) { // 新增
				...
				tbcm011.setCreateEmp(AppUtils.getLoginUserName());

				tbcm011.setPkBs016(AppUtils.getUUIDKey());
				tbcm011.setCompId(AppUtils.getLoginUser().getCompanyId());
				...
				tbcm011.setModifyEmp(AppUtils.getLoginUserName());// 更新修改人
	...
	public String saveTbcm011(Tbcm011 tbcm011) {
		try {
			...
			if (entityState.equals(EntityState.NEW)) { // 新增
				...
				String taxNumber = tbcm011.getTaxNumber();
//				String custNo = null;
//				 custNo = callController.searchCustNo(taxNumber);
//				if (AppUtils.isNotEmpty(custNo)) {
//					tbcm011.setCustNo(custNo);
//				} else {
//				if (tbcm011.getCustNo().isEmpty()) {
				if (tbcm011.getCustNo()==null||tbcm011.getCustNo()=="") {
					String compId = AppUtils.getLoginUser().getCompanyId(); // 公司别
					String noId = "TBCM011_A1"; // 单号代码
//					String ruleString = "c1#s5"; // 规则字串
//					String ruleConstant = "9"; // 字串常数
					String ruleString = "c2#s7"; // 规则字串
					String ruleConstant = "CN"; // 字串常数
					// c:字串常数;p:变量;y:西洋年;m:月份;d:日;s:流水号
					dao.ruleNoDef(compId, noId, ruleString, ruleConstant);
					String no = dao.getNewNo(noId);

					tbcm011.setCustNo(no);
				}
//				}
				...
				tbcm011.setUpdEmp(AppUtils.getLoginUserName());// 更新修改人
		...
		try {
			...
			for (Tbcm012 tbcm012 : dataList) {
				...
				if (entityState.equals(EntityState.NEW)) { // 新增
					...
					tbcm012.setCompId(AppUtils.getLoginUser().getCompanyId());
					tbcm012.setCreateEmp(AppUtils.getLoginUserName());
					...
					// 修改
					tbcm012.setUpdEmp(AppUtils.getLoginUserName());
		...
		try {
			...
			for (Tbbs012 tbcm012 : dataList) {
				...
				if (entityState.equals(EntityState.NEW)) { // 新增
					...
					// 修改
					tbcm012.setModifyEmp(AppUtils.getLoginUserName());
