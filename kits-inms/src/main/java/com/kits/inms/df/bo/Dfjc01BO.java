...
@Controller
public class Dfjc01BO extends HibernateDao {
	...
	public void queryDf01(Page<Tbdf01> page, String pid) {
		...
		if (AppUtils.isNotEmpty(pid)) {
			sql = " from Tbdf01 where def2 ='" + pid + "' and type='A' ";
		} else {
			sql = "  from Tbdf01 where def2 is null";
		}
		
//		sql += " and type='A'  order by createDate asc,updDate desc";
		...
		for (Tbdf01 ch01 : dataList) {
			...
			if (state.equals(EntityState.NEW)) {
				...
				String sql = "update Tbdf02 set ref_id_='" + refId
						+ "' where  ref_id_='" + refIdOld + "' and type_='A' ";
